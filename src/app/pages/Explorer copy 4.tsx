...
function extractData(data) {
  ...
  if (Array.isArray(data)) {
    return Object.fromEntries(data.map(d => [d.key, d["#text"]]));
  }
...
//   const worker = new Worker(new URL('./graphmlWorker.js', import.meta.url));
...
//     cy.add(event.data);
//     cy.layout({
//       name: 'cose-bilkent', // better for large graphs
//       animate: false
//     }).run();
//   };
// }
...
const loadGraphML = async (file: File) => {
...
const filteredEdges = useMemo(() => {
  return consensusEdges.filter(edge => {
    const supportingAlgos = Object.keys(edge.algorithms)
      .filter(algo => selectedAlgorithms.includes(algo));

    const supportPass =
      supportingAlgos.length >= minSupport;

    const scorePass =
      edge.consensus_score >= scoreThreshold;

    return supportPass && scorePass;
  });
}, [consensusEdges, selectedAlgorithms, minSupport, scoreThreshold]);
...
const fetchConsensusEdges = async () => {
  return new Promise<GRNEdge[]>((resolve) => {
    setTimeout(() => {
      resolve(mockConsensusEdges);
    }, 500);
  });
};

useEffect(() => {
  fetchConsensusEdges().then(setConsensusEdges);
}, []);
...
const runLouvain = () => {
  ...
  const d = nodeImportance[node.id()] || 1;
  node.style({
    width: 20 + d * 5,
    height: 20 + d * 5
  });
...
export const Explorer = () => {
  ...
  const [minSupport, setMinSupport] = useState(2);
  const [consensusEdges, setConsensusEdges] = useState(mockEdges);
  ...
  const filteredEdges = useMemo(() => {
    return consensusEdges.filter(edge => {
      const supporting = edge.supporting_algorithms
        .filter(a => selectedAlgorithms.includes(a));

      return (
        supporting.length >= minSupport &&
        edge.consensus_score >= scoreThreshold[0]
      );
    });
  }, [consensusEdges, selectedAlgorithms, minSupport, scoreThreshold]);
  ...
  //   const mods = computeModules(nodeIds, 
  //     // filteredEdges
  //   );
            ...
            <li key={gene}>
              ...
              />   */}
                ...
                {/* <CytoscapeComponent
                  ...
                  elements={graphElements.length > 0 ? graphElements : cytoscapeElements}
                ...
                <CytoscapeComponent
                  cy={(cy) => {
                    if (!cyRef.current) {
                      ...
                      cy.on("tap", "node", (evt) => {
                        setSelectedNode(evt.target.data());
                        highlightNeighbors(cy, evt.target.id(), hopDepth);
                      });
                    }
                  }}
                  elements={graphElements.length > 0 ? graphElements : cytoscapeElements}
                  ...
                  stylesheet={cytoscapeStylesheet}
                  // layout={{
                    // name: graphElements.length > 0 ? "cose" : layout,
                    // animate: false
                  // }}
                  layout={{
                    name: graphElements.length > 0 ? "preset" : layout,
                    animate: false
                  }}
